...
#        post_tensor = torch.cat(post_reps,dim=0).unsqueeze(0)
        ...
        if self.seq:
            ...
            post_tensor = self.linear1( sig( self.linear0( embedding_tensor.mean(dim=1)  ) ) )
...
class Average_BERT_propensity_model():
    ...
    def __init__(self, n_it = 100000, val_interval = 1000, batch_size = 1, lr = 0.001, h_size_sent = 768, h_size_user = 768, tokenize = True, max_tokens_batch = 10000,
                 precalc_path = None, experiment_name = 'hbert', PW = False, seq = False):
        
        self.hb = BERT_avg(h_size_sent=h_size_sent, h_size_user=h_size_user, tokenize = tokenize, max_tokens_batch = max_tokens_batch, PW = PW, seq=seq)
    ...
    def fit(self, dataset, verbose = True, preembed_size = 10):
        self.learning_curve = []
...
#                print('preembed time')
                ...
                for j, iteration in enumerate(iterations):
                    ...
                    if False:# self.precalc_path is not None:
                        ...
                        logit = self.hb.forward(posts_, preembedded[j])
